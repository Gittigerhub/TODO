package com.muzik.todo.Service;

import com.muzik.todo.DTO.TodoDTO;
import com.muzik.todo.Entity.TodoEntity;
import com.muzik.todo.Repository.TodoRepository;
...
public class TodoService {
    ...
    // 서비스에서 작업한 내용을 최종 Repository가 동작
    private final TodoRepository todoRepository;

    // 컨트롤러(DTO)에서 값을 전달받아서 Repository(Entity)에 작업
    // Repository(Entity)로 값을 받아서 컨트롤러(DTO)로 전달
    private final ModelMapper modelMapper;

    /*---------------------------------------------------------------
        함수명 : Page<TodoDTO> a(Pageable pageable)
        인수 : 페이지번호
        결과 : Page<TodoDTO>
    ...
    public Page<TodoDTO> a(Pageable page) {
        ...
        int currentPage = page.getPageNumber()-1;   // 화면에 페이지번호를 데이터베이스 페이지 번호와 맞춰줌
        ...
        // todoEntities에서 하나의 todoEntity를 읽어서 data에 저장
    ...
    /*---------------------------------------------------------------
        함수명 : TodoDTO b(Integer id)
        인수 : 기본키 일련번호
        결과 : TodoDTO
        내용 : 테이블에서 해당번호를 조회해서 데이터를 읽어와 컨트롤러에 전달
    ---------------------------------------------------------------*/
    ...
    public void d(TodoDTO todoDTO) {
        ...
        // 유효성 검사
        // id 값으로 테이블에서 조회해서 레코드를 읽어온다. (있으면 레코드, 없으면 null)
